...
export async function templatesRoutes(app: FastifyInstance) {
  ...
  app.post('/:id/duplicate', { preHandler: requirePermission('templates:create') }, async (request) => {
    const team = requireTeam(request);
    const { id } = request.params as { id: string };
  ...
  app.post('/:id/versions', { preHandler: requirePermission('templates:update') }, async (request) => {
    const team = requireTeam(request);
    const { id } = request.params as { id: string };
  ...
  app.put('/:id/versions/:versionId/restore', { preHandler: requirePermission('templates:update') }, async (request) => {
    const team = requireTeam(request);
