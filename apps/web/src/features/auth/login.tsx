...
export function LoginPage() {
  const { user, isLoading, isFirstTimeSetup, loginWithEmail } = useAuth();
  ...
  // Redirect to register if this is a first-time setup
  useEffect(() => {
  ...
  // Show loading while checking auth status or setup status
  ...
  const oauthRedirect = redirectPath !== '/' ? `?redirect=${encodeURIComponent(redirectPath)}` : '';
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="flex gap-3 mb-6">
          <a
            href={`/api/auth/google${oauthRedirect}`}
          ...
          <a
            href={`/api/auth/github${oauthRedirect}`}
            className="flex-1 border border-gray-300 rounded-lg py-3 px-4 flex items-center justify-center gap-2 hover:bg-gray-50 transition"
          >
